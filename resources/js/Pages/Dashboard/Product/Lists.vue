    ...
    <layout>
        ...
        <div class="container-fluid p-0">
                    ...
                    <div class="card">
                        <div class="card-header d-flex justify-content-between">
                            ...
                            <div class="d-flex justify-content-start">
                                <input type="search" v-model="searchValue" class="form-control mx-2"
                                    placeholder="Search Product">
                                <select v-model="selectedCategory" id="categoryList"
                                    class="js-example-basic-single form-control mx-2" name="state">
                                    <option value="" selected>Select Category</option>
                                    <option :value="category.value" v-for="category in categoryList">{{category.label}}
                                    </option>
                                </select>

                            </div>
                            <button @click="addProductModal = !addProductModal"
                                class="btn mb-2 btn-pill btn-primary">Add New</button>
                        </div>

                        <EasyDataTable :headers="headers" :items="items" :loading="loading"
                            table-class-name="product-table" class="hide-pagination-table" :search-field="searchField"
                            :search-value="searchValue" hide-footer="false" show-index alternating must-sort>
                            ...
                            <template #item-name="{ name,id ,image,unit_size,unit}">
                                <div class="player-wrapper d-flex align-center">
                                    <div class="ms-2 text-decoration-none ">
                                        <a :href="route('products.show',id)" title="View Product" target="_blank"
                                            class="flex mx-1 my-1 no-underline justify-start items-center">
                                            <img :src="image" class="h-10 w-10 rounded-lg shadow-sm">
                                            <div class="flex flex-col">
                                                <span class="text-subtitle-1 mb-0 px-1 text-indigo-500 font-bold">{{
                                                    name }}</span>
                                                <span class="text-subtitle-2 mb-0 px-1 text-xs text-gray-800">{{
                                                    unit_size }} {{unit}}</span>
                                            </div>
            ...
            <div v-if="addProductModal"
                class="fixed inset-0 px-2 z-10 overflow-hidden flex items-center justify-center backdrop-blur-sm transition-opacity ">
                ...
                <div class="w-full sm:w-96 md:w-1/2 lg:w-2/3 bg-white rounded-md shadow-xl overflow-hidden z-50">
                            ...
                            <div class="row mx-1 border border-gray-100 rounded-md">
                                ...
                                <div class="row">
                                    <div class="col-3 mb-2">
                                        <label class="col-form-label text-sm-right">Category <span
                                                class="text-red-500">*</span></label>
                                    </div>
                                    <div class="col-8">
                                        <select v-model="product.category_id" class="form-control" name="categoryLs">
                                            <option value="" selected>Select Category</option>
                                            <option :value="category.value" v-for="category in categoryList">
                                ...
                                <div class="row">
                                    <div class="col-3">
                                        <label class="col-form-label text-sm-right">Name <span
                                                class="text-red-500">*</span></label>
                                    </div>
                                    <div class="col-8">
                                        <input v-model="product.name" class="form-control form-control-lg mb-2"
                                            type="text" placeholder="Product Name">
                                    </div>
                                </div>
                                <div class="row">
                                    <div class="col-3">
                                        <label class="col-form-label text-sm-right">Brand<span
                                                class="text-red-500">*</span></label>
                                    </div>
                                    <div class="col-8">
                                        <multiselect v-model="product.brand_id" tag-placeholder="Add this as new tag"
                                            placeholder="Search or add a tag" :options="brandOptions" :multiple="false"
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-8">
                                        <input v-model="product.type" class="form-control form-control-lg mb-2"
                                ...
                                <div class="w-50 flex justify-end items-center">
                                    <div class="w-full  my-2">
                                        <!-- Upload Container -->
                                        <div @dragover.prevent @dragleave.prevent @drop.prevent="handleDrop" :class="['w-full flex justify-between items-center hover:bg-green-100',
                                            ...
                                            <div class="my-2 mx-1">
                                                ...
                                                <div class="w-full">
                                                    ...
                                                    <p class="text-xs text-gray-500"> Click to upload or drag and
                                        ...
                                        <div v-if="uploading" class="mt-4">
                                            ...
                                            <p class="text-sm text-gray-600 mt-2">Uploading... {{ uploadProgress }}%
                                            </p>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <div class="border-t px-4 py-2 flex justify-end">
                        ...
                        <button v-if="product.id" @click="updateProduct()" type="button"
                            class="px-3 py-1  mx-2 text-white  rounded-md w-full sm:w-auto  bg-indigo-500"
                            data-bs-dismiss="modal">Update</button>
                        <button v-if="!product.id" @click="addProduct()" type="button"
...
<script setup >
        ...
        try {
            ...
            if (response.status === 200) {
                ...
                items.value = response.data.data.map( (item,i) => ({
                    id: item.id, // Keep the product detail id for actions
    ...
    const editProduct = (data) => {
        // Handle edit action
        addProductModal.value = true; console.log(data);
        product.value = {
            id: data.id,
            name: data.name,
            type: data.type,
